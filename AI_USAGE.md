...
What happens _after_ the embeddings is fully deterministic:
...
| Transcript cleanup utilities ([src/clean*transcript*\*](src/), [src/extract_min10y_transcripts.py](src/extract_min10y_transcripts.py))                                 | Heavy                                   | Spot-checked on per-ticker output                                         |
...
| Quarto write-up prose ([reports/writeup_v2.qmd](reports/writeup_v2.qmd)), [METHODOLOGY.md](METHODOLOGY.md)                                                             | Moderate (drafting), heavy (formatting) | Every numerical claim verified against the rendered notebook / JSON files |
...
| Numerical claims in the write-up | Every number quoted in `reports/writeup_v2.qmd` is loaded at render time from `_data/metrics_*.json`, `_data/ic_summary.json`, `_data/fm_results.json`, or `_data/factor_alpha.json`. There are no hand-typed numbers in the prose. |
