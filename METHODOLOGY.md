...
*Draft — methodology cleanup. Charts and final numbers will be merged in from
`_output/99_results.ipynb` for the final deliverable.*
...
strategy on a 91-ticker Nasdaq-100 universe with at least ten years of
transcript history.

Five signals are compared on identical backtest machinery (monthly rebalance,
top-20 / bottom-20 equal-weighted legs, 21-trading-day holding period, no
transaction costs):
...
4. **Price momentum 12-1** — the standard equity factor; included as a
   non-text benchmark.
5. **Analyst revisions in BEst Net Income** — 30-day estimate revisions
   from Bloomberg; the second non-text benchmark.
...
The investable universe is **91 Nasdaq tickers** drawn by applying a
"minimum-10-year transcript history" filter to the current Nasdaq-100
membership. The list is fixed across the entire backtest period: there is no
point-in-time index membership adjustment. Every signal runs against the
same 91 names every month.
...
comparison across signals — they all see the same 91 tickers — but symmetry
of bias across signals is an assumption, not a verified property. This is
named in the limitations section and is not corrected.

### 3.2 Transcripts

For each of the 91 tickers, full earnings-call transcripts were pulled from
...
Daily prices for the 91 tickers and Bloomberg `BEst Net Income` consensus
...
| Parameter | Value |
|---|---|
| Universe | 91 Nasdaq tickers (Section 3.1) |
| Rebalance frequency | Monthly (last business day of month) |
| Ranking | Cross-sectional rank of the signal across all 91 tickers |
| Long leg | Top 20 tickers by signal value |
| Short leg | Bottom 20 tickers by signal value |
| Weighting | Equal-weight within each leg (5 % per stock) |
...
**Why `MIN_OBS_PER_MONTH = 40`.** Sorting top-20 / bottom-20 out of a
cross-section smaller than ~40 starts to produce overlap between the two
legs and degenerate rankings. Forty is the floor below which we report
"no portfolio formed" for that month rather than carry through a malformed
rebalance.
...
| Information ratio vs benchmark | $\text{mean}(r_{LS} - r_{\text{eqweight-91}}) \cdot 12 \big/ \text{std}(r_{LS} - r_{\text{eqweight-91}}) \cdot \sqrt{12}$ |

The benchmark for the information ratio is the equal-weighted return of
the 91-ticker universe, not the cap-weighted Nasdaq-100 — internal
consistency with how the legs are weighted.
...
Portfolio return depends only on the **tails** of the ranking — the top 20
and the bottom 20. A signal that gets one or two big winners right can
post a positive long-short return even if the middle 50 stocks are
randomly ordered. IC measures the **full ordering**. The two views answer
different questions:
...
- **Survivorship bias.** 91 tickers chosen as the current Nasdaq-100 with
  ≥10-year transcript history. Names that delisted, were acquired, or fell
  out of the index do not appear. Bias should largely (but not perfectly)
  cancel in *relative* comparison across signals.
- **Small universe.** 91 tickers and 20-name quintile legs limit statistical
  power and concentrate exposure. A claim like "Sharpe of strategy A is
  twice strategy B's" is much weaker on 91 names than on the Russell 3000.
...
| Quintile legs | 20-long / 20-short of 91 | `backtest.py` | Top / bottom 22 % of universe |
