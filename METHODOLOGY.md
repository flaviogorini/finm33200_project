...
- **Does the _language_ of corporate disclosure carry cross-sectional return
...
and a single embedding pooling strategy (call-level mean). What we _can_
...
about deployable performance. They do not preclude meaningful _relative_
...
- **S&P Global Market Intelligence — _Familiar signal, new context: The
  evolution of earnings call sentiment analysis from lexicons to LLMs_
...
- **Ghosal (2026) — _LLM-Driven Investment Models: Evidence from Earnings
  Call Transcripts_.** Uses 2019–2023 transcripts and sentence-transformer
...
- **Loughran and McDonald (2011) — _When is a Liability not a Liability?_** The
...
- **Glasserman and Lin (2023) — _Assessing Look-Ahead Bias in Stock Return
  Predictions Generated by GPT Sentiment Analysis._** Cautions about the
...
the index do not appear. Bias should largely cancel in the _relative_
...
operations, some more about R&D) and isolates the _new_ language of the
quarter.
...
`days_since_earnings` is **concatenated _after_ the PCA**, not put through
...
sits at the top of the grid is _not_ the same problem as in the raw-1,537
...
**The Bloomberg field is _blended forward_, not FY1.** The
...
that month is **dropped** from the cross-section _for the sentiment
strategies only_ (anchor, ridge, LM). Momentum and revisions are unaffected
...
return across the cross-section that month. This is a _single number per
month_ describing whether the full ordering of stocks lined up with the
...
- **Survivorship bias.** 100 tickers drawn from current Nasdaq-100
  ...
  _relative_ comparison across signals.
...
### 11.1 Strategy 6 — CAR3 (earnings-window cumulative return)

For each earnings call at `event_date` $t$, define

$$\mathrm{CAR3}_{i,t} = \frac{p_{i, t+1}}{p_{i, t-1}} - 1$$

where $p_{i, t\pm 1}$ is the closing price one trading day after / before
the announcement. Computed by `src/build_factor_baseline.py` via
`calendar_utils.fwd_ret_bd(prices, event_date, h=2, gap=-1)` (entry at $t-1$,
exit at $t+1$). Saved as `_data/car3_per_call.parquet`
(`ticker, event_date, car3`).

The per-call CAR3 is carried forward to monthly rebalance dates per ticker
using the same `_carry_forward_per_call` mechanism that anchor/ridge/LM
use (§4.8). Stored in `_data/signal_panel_monthly.parquet` as `sig_car3`.
Eligible for the `days_since_earnings > 60` stale-call filter under the
`stale_excl` spec (CAR3 is in `TRANSCRIPT_STRATEGIES` set in
`run_backtests.py`).

| Hyperparameter           | Value                                        | Rationale                                                                                    |
| ------------------------ | -------------------------------------------- | -------------------------------------------------------------------------------------------- |
| CAR window               | `[event_date − 1 BDay, event_date + 1 BDay]` | Literature-standard $[-1, +1]$ announcement-window CAR                                       |
| Net-of-market adjustment | None (raw CAR)                               | Matches project convention; equal-weight benchmark in §4 already normalises for market drift |
| Carry-forward window     | 60 calendar days                             | Same as anchor/ridge/LM; matches the post-earnings decay horizon                             |

### 11.2 Signal correlation matrices (writeup §5.4)

Two 6×6 correlation tables are computed inline by
`src/99_results_v2.ipynb.py` (no separate pipeline script) and persisted as
small JSON artefacts under `_data/`:

**Stock-level Spearman.** Z-score each signal within each rebalance month
(matches §5.5 preprocessing), then compute the pooled Spearman rank
correlation across all (ticker, month) cells. Saved as
`_data/signal_corr_stock.json`. Diagnoses joint-FM multicollinearity.

**Portfolio-level Pearson.** Pivot `_data/results_main.parquet` to wide form
`date × strategy → ret_ls`, then compute the Pearson correlation matrix of
the monthly LS return series. Saved as `_data/signal_corr_portfolio.json`.
Diagnoses §5.6 α-regression multicollinearity.

Pairs with $|r| > 0.5$ are flagged inline in the notebook for the writeup.

### 11.3 Nested time-series α progression (writeup §5.6)

For each text strategy $k \in \{\text{Ridge}, \text{Anchor}, \text{LM}\}$,
fit four nested OLS regressions of the strategy's monthly LS return on a
growing factor set. Implemented in `src/factor_regression.py`. Output
`_data/factor_alpha.json` with two top-level sample windows
(`own_history`, `post_2018`).

**Baseline factor set.** Fama-French 5 factors from Ken French data
library — Mkt-RF, SMB, HML, RMW, CMA. Downloaded over HTTP by
`src/build_factor_baseline.py` (no auth); cached in
`_data/ff5_monthly.parquet` in decimal form (divided by 100 from the
percent-units source). The Carhart momentum factor is intentionally NOT
included in the baseline — momentum enters as the project's own Strategy
4 LS return in the nested progression.

**Nested specs.**

| Key       | RHS factors                                    | What is added                                |
| --------- | ---------------------------------------------- | -------------------------------------------- |
| `alpha_0` | FF5 (Mkt-RF, SMB, HML, RMW, CMA)               | —                                            |
| `alpha_1` | FF5 + `mom_strat`                              | + Momentum strategy LS as a factor portfolio |
| `alpha_2` | FF5 + `mom_strat` + `car3_strat`               | + CAR3 strategy LS                           |
| `alpha_3` | FF5 + `mom_strat` + `car3_strat` + `rev_strat` | + Revisions strategy LS                      |

For each (LHS strategy, RHS spec, sample) triple, report $\alpha$ (mean
monthly residual return) and its NW-HAC $t$-statistic (lag 6, consistent
with §4.10). The LHS strategy is excluded from the RHS even if it would
otherwise appear there (defensive, no-op for the three text strategies
which are not in `FACTOR_STRATEGIES`).

**Timestamp alignment.** Strategy LS at signal-date $T$ earns its return
during the following calendar month. We align by shifting the FF5 dates
backward one business-month-end so that FF5's row originally for month
$T+1$ is keyed at $T$ and merges cleanly with the strategy LS at $T$.
Implemented as `ff5["date"] = ff5["date"] - pd.offsets.BMonthEnd(1)` in
`load_ff5_shifted`. The ~1-2 business-day boundary mismatch at month-end
is documented but not corrected; it affects $\alpha$ magnitudes by well
under 5% and does not change signs or t-statistics.

**No risk-free-rate subtraction.** Strategy LS portfolios are zero-cost
long-short; subtracting RF from the LHS would be mechanically inappropriate
(no funding cost). FF5's `MKT-RF` on the RHS is already in excess-return form.

**Sample windows.**

- `own_history`: each LHS strategy's own backtest start onward (LM and
  Anchor extend back to ~2006-2008; Ridge starts Feb 2019).
- `post_2018`: restrict to date ≥ `2018-12-31`, matching the
  `task_run_backtests` `post2018` spec.

### 11.4 Pipeline additions

New doit tasks (additive; existing tasks unchanged):
