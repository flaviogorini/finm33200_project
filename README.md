...
FINM 33200 final project. A comparative backtest of six cross-sectional long-short equity signals on a fixed 100-ticker Nasdaq-100 universe, identical machinery across all six, isolating the effect of signal choice from backtest mechanics.

The central question: **do off-the-shelf LLM embeddings of earnings-call transcripts add cross-sectional return information beyond a Loughran-McDonald lexicon and the standard non-text factors (price momentum, analyst revisions, earnings-window returns)?**

The six signals, all run on identical machinery (monthly rebalance, top-quintile / bottom-quintile equal-weighted, 21-trading-day holding period, no transaction costs):
...
6. **CAR3 — earnings-window cumulative return** — 3-trading-day cumulative return centred on the announcement date.
...
- [`reports/writeup_v2.html`](reports/writeup_v2.html) — the audience-facing write-up. Motivation, signal taxonomy, headline results, robustness checks (including nested α progression), limitations. Self-contained HTML; open it directly.
- [`METHODOLOGY.md`](METHODOLOGY.md) — methodology spec. Every transformation, hyperparameter, and choice with rationale.
- [`_output/99_results_v2.html`](_output/99_results_v2.html) — auto-rendered results notebook with all tables and charts.
...
| Path                                  | What's in it                                                                                                                                  |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| [`src/`](src/)                        | Pipeline modules (one responsibility per file) + `99_results_v2.ipynb.py` (jupytext notebook).                                                |
...
python -m doit write_report        # re-render reports/writeup_v2.html
```
...
- `_output/99_v2_*.png` — six charts: hit rates, rolling IC, cumulative returns and drawdowns on two common windows.
- `_output/99_results_v2.html` — auto-rendered jupyter notebook with all tables and charts.
- `reports/writeup_v2.html` — final audience-facing write-up (self-contained, all images embedded).
